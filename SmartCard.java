...
/**
 * GYM SMART CARD - PHASE 2.1: AES-128-CBC WITH RANDOM IV
 * 
 * Card Structure (96 bytes) - SECURE (PII ENCRYPTED with CBC):
 * [0-1]   UserID (2 bytes)
 * [2-17]  IV (16 bytes, random for each write)
 * [18-65] Encrypted Block (48 bytes AES-128-CBC NoPadding)
 *          Plaintext layout inside 48 bytes:
 *            - Balance (4)
 *            - ExpiryDays (2)
 *            - DOB Day (1)
 *            - DOB Month (1)
 ...
 * [67-82] PIN Hash (16 bytes) - SHA-256 truncated to 16 bytes
 * [83-95] Reserved zeros
 */
public class SmartCard extends Applet {
    // Data offsets - CBC with IV
    private static final byte OFFSET_USER_ID = 0;
    private static final byte OFFSET_IV = 2;                // 16-byte IV
    private static final byte OFFSET_ENC_BLOCK = 18;        // Start of encrypted block (after IV)
    private static final byte ENC_BLOCK_LEN = 48;           // 48 bytes (3 AES blocks)
    private static final byte OFFSET_PIN_RETRY = 66;        // After IV + encrypted block
    private static final byte OFFSET_PIN_HASH = 67;         // 16-byte PIN hash follows retry
    // PII stored inside encrypted block; no public offsets.
    
    private static final short DATA_SIZE = 96;
    ...
    public SmartCard() {
        ...
        tempBuffer = JCSystem.makeTransientByteArray((short) 96, JCSystem.CLEAR_ON_DESELECT);
    ...
    private void handleWrite(APDU apdu) {
        ...
        // Copy fields (CBC layout with IV)
        // UserID [0-1]
        Util.arrayCopyNonAtomic(buf, ISO7816.OFFSET_CDATA, cardData, (short) 0, (short) 2);
        // IV [2-17] - 16 bytes from client (random IV generated by PC)
        Util.arrayCopyNonAtomic(buf, (short)(ISO7816.OFFSET_CDATA + OFFSET_IV), cardData, OFFSET_IV, (short) 16);
        // Encrypted block [18-65] - 48 bytes
        Util.arrayCopyNonAtomic(buf, (short)(ISO7816.OFFSET_CDATA + OFFSET_ENC_BLOCK), cardData, OFFSET_ENC_BLOCK, ENC_BLOCK_LEN);
        // PIN retry [66]
    ...
    private void handleVerifyPIN(APDU apdu) {
        ...
        if (match) {
            ...
            // Decrypt the 48-byte encrypted block using CBC with IV from cardData[2-17]
            aesCipher.init(aesKey, Cipher.MODE_DECRYPT, cardData, OFFSET_IV, (short) 16);
            aesCipher.doFinal(cardData, OFFSET_ENC_BLOCK, ENC_BLOCK_LEN, tempBuffer, (short) 0);

            // Copy all card data to buffer
            Util.arrayCopyNonAtomic(cardData, (short) 0, buf, (short) 0, DATA_SIZE);

            // Replace encrypted block with decrypted data (keep IV in place)
            Util.arrayCopyNonAtomic(tempBuffer, (short) 0, buf, OFFSET_ENC_BLOCK, ENC_BLOCK_LEN);
    ...
    private void handleChangePIN(APDU apdu) {
        ...
        // ✅ RE-ENCRYPT 48-byte sensitive block with NEW PIN and NEW IV
        // 1. Decrypt with OLD PIN using existing IV
        ...
        aesCipher.init(aesKey, Cipher.MODE_DECRYPT, cardData, OFFSET_IV, (short) 16);
        aesCipher.doFinal(cardData, OFFSET_ENC_BLOCK, ENC_BLOCK_LEN, tempBuffer, (short) 32);

        // 2. Generate new random IV
        randomGenerator.generateData(cardData, OFFSET_IV, (short) 16);
        
        // 3. Encrypt with NEW PIN and NEW IV
        deriveAESKeyFromPIN(buf, newPinOffset, (short) 6);
        aesCipher.init(aesKey, Cipher.MODE_ENCRYPT, cardData, OFFSET_IV, (short) 16);
        aesCipher.doFinal(tempBuffer, (short) 32, ENC_BLOCK_LEN, cardData, OFFSET_ENC_BLOCK);
